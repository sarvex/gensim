...
def bump_setup_py(root, previous_version, new_version):
    ...
    pattern = re.compile(f"^    version='{previous_version}',$", re.MULTILINE)
...
def bump_docs_src_conf_py(root, previous_version, new_version):
    ...
    pattern = re.compile(f"^version = '{short_previous_version}'$", re.MULTILINE)
    repl = f"version = '{short_new_version}'"
    bump(path, pattern, repl, check=False)  # short version won't always change

    pattern = re.compile(f"^release = '{previous_version}'$", re.MULTILINE)
    repl = f"release = '{new_version}'"
    bump(path, pattern, repl)


def bump_gensim_init_py(root, previous_version, new_version):
    path = os.path.join(root, 'gensim', '__init__.py')
    pattern = re.compile(f"__version__ = '{previous_version}'$", re.MULTILINE)
